...
export async function POST(req: Request) {
  try {
    ...
    const userResult = await sql`SELECT * FROM public.users WHERE email = ${email}`;
    let user = userResult[0];

    if (!user) {
      const insertedUser = await sql`INSERT INTO public.users (email) VALUES (${email}) RETURNING id, email`;
      user = insertedUser[0];
    }
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: "Link de acesso Smart Plant APP",
      html: `
        <h2>Smart Plant APP</h2>
        <p>Olá ${user.name},</p>
        ...
        <hr>
        <small>Smart Plant &copy; ${new Date().getFullYear()}</small>
      `,
    });
